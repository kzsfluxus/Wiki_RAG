...
class Embedder:
    ...
    def build_index(self, docs):
        ...
        print(
            f"📄 Első dokumentum: {docs[0].get('text', 'NINCS TEXT MEZŐ')[:100]}...")
    ...
    def save(self, index_path='data/index.faiss',
             docs_path='data/wiki_pages.json'):
    ...
    def load(self, index_path='data/index.faiss',
             docs_path='data/wiki_pages.json'):
        try:
            ...
            print(
                f"📂 Betöltve: {len(self.documents)} dokumentum, {self.index.ntotal} vektor")
    ...
    def query(self, question, top_k=3):
        ...
        for i, idx in enumerate(indices[0]):
            if idx >= 0 and idx < len(
                    self.documents):  # Érvényes index ellenőrzés
                doc = self.documents[idx]
                logger.debug(
                    f"📄 {i+1}. találat (távolság: {distances[0][i]:.3f}): {doc.get('text', '')[:100]}...")
                results.append(doc)
            else:
