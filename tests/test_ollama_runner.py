...
def test_run_ollama_model_success(mock_run):
    mock_run.return_value = subprocess.CompletedProcess(
        args=['ollama', 'run', 'mistral-openorca'],
        returncode=0,
        stdout=b'Model response here.',
        stderr=b''
    )

    result = ollama_runner.run_ollama_model("Hello", model_name="mistral-openorca")
...
def test_stop_ollama_model_success(mock_run):
    mock_run.return_value = subprocess.CompletedProcess(
        args=['ollama', 'stop', 'mistral-openorca'],
        returncode=0,
        stdout=b'Stopped successfully.',
        stderr=b''
    )

    result = ollama_runner.stop_ollama_model(model_name="mistral-openorca")
